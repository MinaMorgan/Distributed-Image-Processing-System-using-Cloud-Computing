...
class WorkerThread(threading.Thread):
    ...
    def run(self):
        while True:
            ...
            result = self.process_image(image_path, text, operation)
            self.send_result(result)

    def process_image(self, image_path, text, operation):
            ...
            for i, vm_ip in enumerate(online_vms):
                ...
                try:
                    if operation in ['read_qr_code', 'resize', 'equalize_histogram']:
                        if i==0:
                            one_operation_response = executor.submit(post_data, base_url.format(vm_ip), img, text, operation)
                    else:
                        future = executor.submit(post_data, base_url.format(vm_ip), img_part, text, operation)  # Pass text to post_data
            ...
            if not (all(response and response.status_code == 200 for response in responses) and (len(responses) == len(online_vms))):
                ...
                for new_vm_ip in online_vms:
                    if new_vm_ip != failed_ip:
                        ...
                        new_response = post_data(base_url.format(new_vm_ip), temp_img, text, operation)  # Pass text to post_data
        ...
        if operation in ['read_qr_code', 'resize', 'equalize_histogram']:
            ...
            new_response._content = json.dumps(one_operation_response.result().json()).encode('utf-8')
        else:
...
def post_data(url, image, text, operation):
    response = requests.post(url, json={"image": image.tolist(), "text": text, "operation": operation})
