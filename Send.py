...
class ImageProcessor(ctk.CTk):
    ...
    def create_widgets(self):
        ...
        self.select_image_button = ctk.CTkButton(
            self, text="Select Image", width=200, command=self.select_image
        )
        ...
        self.operation_combobox = ctk.CTkComboBox(
            self, width=140, values=["edge_detection", "color_inversion"]
        )
        self.operation_combobox.set(
            ""
        )  # Set the empty string as the default selected value
        self.operation_combobox.place(x=280, y=120)

        # Button for sending requests
        self.send_request_button = ctk.CTkButton(
            self, text="Send Request", width=200, command=self.send_request
        )
    ...
    def send_request(self):
        url = "http://16.170.171.117:5000/process"
        ...
        try:
            ...
            data = {"text": text, "operation": operation}
